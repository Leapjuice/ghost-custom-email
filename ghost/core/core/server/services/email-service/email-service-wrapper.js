...
class EmailServiceWrapper {
    ...
    init() {
        ...
        const SendGridClient = require('../lib/sendgrid-client');
        ...
        // Get the email provider type from settings
        const emailProviderType = settingsCache.get('email_provider') || 'mailgun';

        let emailProvider;
        let providerName;

        if (emailProviderType === 'sendgrid' && sendGridClient.isConfigured()) {
            emailProvider = new SendGridEmailProvider({
                sendGridClient,
                errorHandler
            });
            providerName = 'SendGrid';
            logging.info('Using SendGrid as email provider');
        } else if (emailProviderType === 'smtp' && smtpClient.isConfigured()) {
            emailProvider = new SmtpEmailProvider({
                smtpClient,
                errorHandler
            });
            providerName = 'SMTP';
            logging.info('Using SMTP as email provider');
        } else {
            // Default to Mailgun
            ...
            logging.info('Using Mailgun as email provider');
        }
